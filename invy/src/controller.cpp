...
class Controller : public RFModule
{
    ...
    {
        Matrix J(3,3);
        J(0,0)=-link_length*(sin(j[0])+sin(j[0]+j[1])+sin(j[0]+j[1]+j[2])); J(0,1)=-link_length*(sin(j[0]+j[1])+sin(j[0]+j[1]+j[2])); J(0,2)=-link_length*sin(j[0]+j[1]+j[2]);
        J(1,0)=link_length*(cos(j[0])+cos(j[0]+j[1])+cos(j[0]+j[1]+j[2])); J(1,1)=link_length*(cos(j[0]+j[1])+cos(j[0]+j[1]+j[2])); J(1,2)=link_length*cos(j[0]+j[1]+j[2]);
...
//5.0**************************************************/
	...
	//6.0//Vector ee_d=(usePlanner?planner->getPos():target);
	
	//6.0 retrieve the current target orientation
        ...
        if (mode=="t")
        {
            Matrix G=gains[0]*eye(3,3);
            vel=J.transposed()*G*err;
        }
        else if (mode=="inv")
        {
            Matrix G=gains[1]*eye(3,3);
            vel=pinv(J)*G*err;
        }
        else if (mode=="dls")
        {
            double k=10.0;
            Matrix G=gains[2]*eye(3,3);
            vel=J.transposed()*pinv(J*J.transposed()+k*k*eye(3,3))*G*err;
        }
        else
        {
            vel=zeros(3);
